...
import DatenbankClass

a = DatenbankClass
datenbank = a.Datenbank("Stoffdatenbank")
datenbank.check()
val = datenbank.value("stoffdatenpropan", "TP" , 2, "hpf")
...
#Gebäudeklasse beinhaltet alle wichtigen Formeln für die Berechneng
class Gebaüde:
    ...
    #Endtemperatur der Luft beim Verlassen des Bueros
    def calc_tEnde(self):
        qIng = self.calcQ_Ing()
        xBuero = self.calcx()
        qWand = self.calcQWand()
        tE = sy.symbols('tE')
        hIn = Mdot_Luft*Cp_Luft*self.tWunsch + Mdot_Luft * xBuero*(Cp_Dampf * self.tWunsch+ Rd)
...
class Propan(Waermeuebertrager):
    ...
    def calcR_Propan(self, T):
        return datenbank.value("stoffdatenpropan", "TP" , T, "hpg") - datenbank.value("stoffdatenpropan", "TP" , T, "hpf")
...
class WaermeübertragerCO2:
    ...
    def calcT2(self, h_in_wü, mCO2):
        return self.t1 + h_in_wü/(self.Cp_CO2*mCO2)

    def calcEpsilons(self, t2, tw, sol_te):
        epsilon1 = (sol_te-tw)/(sol_te-self.t1)
        epsilon2 = (t2-self.t1)/(sol_te-self.t1)
